...
StateWrapper::StateWrapper(size_t state_id, DataWrapper* data) : state_id_(state_id), data_(data) {
    THROW_IF(!data_, ::errors::LogicError, "Pointer to data should not be null.");
}

void StateWrapper::AddEdgeTo(StateWrapper another_state, uint8_t code) {
    ASSERT_NOT_NULL(data_);
    auto real_state = data_->ExtractState(*this);
...
StateWrapper StateWrapper::AddEdge(uint8_t code) {
    ASSERT_NOT_NULL(data_);
    auto another_state = data_->CreateState();
...
size_t StateWrapper::MakeFinal() {
    ASSERT_NOT_NULL(data_);
    auto real_state = data_->ExtractState(*this);
    real_state.final_id_ = data_->NextFinalId();
