...
private:
    ...
    //     std::unique_ptr<ast::Expression> result = ParsePlusMinusExpression();
    //     if (Super::Peek().Is({e::TokenType::EQ_O, e::TokenType::NE_O, e::TokenType::LT_O,
    //             e::TokenType::LE_O, e::TokenType::GT_O, e::TokenType::GE_O})) {
    //         auto token = Super::Consume();
    //         auto right = ParsePlusMinusExpression();
    //         auto type = ast::enums::TokenTypeToBinaryOperation(token.GetType());
    //         result = std::make_unique<ast::expr::BinaryExpression>(
    //             type, std::move(result), std::move(right));
    //     }
    //     return result;
    // }
    //
    // std::unique_ptr<ast::Expression> ParsePlusMinusExpression() {
    //     std::unique_ptr<ast::Expression> result = ParseMulDivExpression();
    //     while (Super::Peek().Is({e::TokenType::PLUS_O, e::TokenType::MINUS_O})) {
    //         auto token = Super::Consume();
    ...
    // }
    //
    // std::unique_ptr<ast::Expression> ParseMulDivExpression() {
    //     std::unique_ptr<ast::Expression> result = ParseLiteral();
    //     while (Super::Peek().Is({e::TokenType::STAR_O, e::TokenType::SLASH_O})) {
    //         auto token = Super::Consume();
    ...
    std::unique_ptr<ast::expr::Literal> ParseLiteral() {
        if (Super::Peek().Is(e::TokenType::INT_LITERAL)) {
            auto token = Super::Consume();
            return std::make_unique<ast::expr::lit::IntLiteral>(token.GetValue());
        }
        return nullptr;
    }
};
...
private:
    using Super = lib::lang::frontend::Parser<e::TokenType, ast::Node, Context>;
